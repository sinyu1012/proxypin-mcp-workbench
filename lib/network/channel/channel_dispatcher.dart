...
class ChannelDispatcher extends ChannelHandler<Uint8List> {
  ...
  Future<void> channelRead(ChannelContext channelContext, Channel channel, Uint8List msg) async {
    ...
    try {
      ...
      // SSE 响应不会结束，因此需要在 isDone=false 时先转发响应头并切换为流式处理。
      // 普通 HTTP 响应必须继续等待 body 解析完成，否则会被错误地当作 SSE，导致正文截断且连接不结束。
      if (decodeResult.data is HttpResponse) {
        var response = decodeResult.data as HttpResponse;
        var isSSE = response.headers.contentType.toLowerCase().startsWith('text/event-stream');
        logger.d(
            "[channelRead] SSE check: isSSE=$isSSE, isDone=${decodeResult.isDone}, data=${decodeResult.data}, remoteChannel=${remoteChannel != null}");

        // SSE 流需要在 isDone=false 时仍然发送响应头给客户端
        // 否则客户端收不到任何响应
        if (isSSE && !decodeResult.isDone && remoteChannel != null) {
          ...
          logger.d("[channelRead] SSE detected, setting up SSE streaming");
          // remoteChannel 是客户端连接，发送响应给客户端
          remoteChannel.write(channelContext, response);

          // 设置后续数据流的处理：
          // 1. channel (服务端) -> SseChannelHandler -> remoteChannel (客户端)
          // 2. remoteChannel (客户端) -> RelayHandler -> channel (服务端)
          // 保存 channel (服务端连接) 和 remoteChannel (客户端连接) 的原始状态
          // 这样 SSE 流结束后可以恢复，让后续 keep-alive 请求能正常被解析和添加到列表
          // channel 原始状态：HttpClientCodec + HttpResponseProxyHandler
          // remoteChannel 原始状态：HttpRequestCodec + HttpResponseCodec + HttpProxyChannelHandler
          final channelOriginalDecoder = channel.dispatcher.decoder;
          final channelOriginalEncoder = channel.dispatcher.encoder;
          final channelOriginalHandler = channel.dispatcher.handler;
          final remoteChannelOriginalDecoder = remoteChannel.dispatcher.decoder;
          final remoteChannelOriginalEncoder = remoteChannel.dispatcher.encoder;
          final remoteChannelOriginalHandler = remoteChannel.dispatcher.handler;
          ...
          channel.dispatcher.channelHandle(
              ...
              SseChannelHandler(
                  remoteChannel,
                  response,
                  channelOriginalDecoder,
                  channelOriginalEncoder,
                  channelOriginalHandler,
                  remoteChannelOriginalDecoder,
                  remoteChannelOriginalEncoder,
                  remoteChannelOriginalHandler));
          remoteChannel.dispatcher.channelHandle(rawCodec, RelayHandler(channel));
          // 把初始 body 显式转发给 SseChannelHandler 来处理（它会转发给客户端）。
          if (response.body != null && response.body!.isNotEmpty) {
            logger.d("[channelRead] SSE: forwarding initial body ${response.body!.length} bytes to handler");
            channel.dispatcher.handler.channelRead(channelContext, channel, Uint8List.fromList(response.body!));
          }
          return;
        }
      }
  ...
  void notSupportedForward(ChannelContext channelContext, Channel channel, DecoderResult decodeResult) {
    ...
    if (decodeResult.data is HttpResponse) {
      var response = decodeResult.data as HttpResponse;
      logger.d(
          "[notSupportedForward] Content-Type=${response.headers.contentType} isSSE=${response.headers.contentType.toLowerCase().startsWith('text/event-stream')} remoteChannel!=null=${remoteChannel != null}");
