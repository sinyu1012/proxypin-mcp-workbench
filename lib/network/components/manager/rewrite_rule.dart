...
class RequestRewriteRule {
  ...
  /// 仅检查 URL，供命中诊断与真实匹配共享同一正则。
  bool matchesUrl(String candidate) => _urlReg.hasMatch(candidate);

  /// 未配置方法表示匹配任意方法；调用方未提供方法时保持历史兼容行为。
  bool matchesMethod(HttpMethod? candidate) => candidate == null || method == null || method == candidate;
  ...
  factory RequestRewriteRule.formJson(Map<dynamic, dynamic> map) {
    ...
    return RequestRewriteRule(
        ...
        name: map['name'],
        group: _normalizedGroup(map['group']),
  ...
  static String? _normalizedGroup(dynamic value) {
