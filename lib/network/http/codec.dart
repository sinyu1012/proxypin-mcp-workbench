  ...
  DecoderResult<T> decode(ChannelContext channelContext, ByteBuf data) {
    ...
    try {
      ...
      if (_state == State.body) {
        ...
        } else if (bodyResult != null && bodyResult.body != null) {
          // For SSE streams (isDone=false but body available), still set body for UI display
          result.data!.body = bodyResult.body;
        }
...
class HttpClientCodec extends Codec<HttpResponse, HttpRequest> {
  ...
  /// 重置 codec 状态
  /// 这是关键的修复：SSE 流期间 codec 的 _state 保持 State.body，
