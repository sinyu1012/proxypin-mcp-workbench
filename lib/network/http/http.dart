...
abstract class HttpMessage {
  ...
  String getBodyString({String? charset}) {
    ...
    try {
      ...
      // 性能修复：SSE 响应的 body 已经是解压后的明文（SseChannelHandler 边收边解压）
      // 不能再用 brDecode 解码（否则会失败或乱码）
      // 判断条件：content-type 为 text/event-stream（SSE），且 content-encoding 为 br
      bool isSseResponse = headers.contentType.toLowerCase().contains('text/event-stream');
      if (!isSseResponse) {
        if (headers.isGzip) {
  ...
  void addMessages(List<WebSocketFrame> frames) {
    ...
    messagesNotifier.value = [...messagesNotifier.value, ...frames];
  }
}
