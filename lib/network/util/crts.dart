...
class CertificateManager {
  ...
  static Future<void>? _caFilesInitialization;
  ...
  static Future<File> certificateFile() async {
    ...
    await _ensureLocalRootCA(appPath);
    return caFile;
  }
  ...
  static Future<void> _ensureLocalRootCA(String appPath) async {
    final certFile = File("$appPath${Platform.pathSeparator}ca.crt");
    final keyFile = File("$appPath${Platform.pathSeparator}ca_key.pem");
    if (await certFile.exists() && await keyFile.exists()) return;

    final pending = _caFilesInitialization;
    if (pending != null) return pending;

    final initialization = _createLocalRootCA(certFile, keyFile);
    _caFilesInitialization = initialization;
    try {
      await initialization;
    } finally {
      if (identical(_caFilesInitialization, initialization)) {
        _caFilesInitialization = null;
      }
    }
  }

  static Future<void> _createLocalRootCA(File certFile, File keyFile) async {
    final templateBytes = await FileRead.read('assets/certs/ca.crt');
    final template = X509Utils.x509CertificateFromPem(String.fromCharCodes(templateBytes));
    ...
    final subject = <String, String>{
      ...
      'O': 'Proxy',
      'OU': 'ProxyPin',
      'CN': 'ProxyPin CA (${DateTime.now().dateFormat()},${RandomUtil.randomString(6).toUpperCase()})',
    };
    ...
    await certFile.writeAsString(certificate, flush: true);
    await keyFile.writeAsString(CryptoUtils.encodeRSAPrivateKeyToPem(privateKey), flush: true);
