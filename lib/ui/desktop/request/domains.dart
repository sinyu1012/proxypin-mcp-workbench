...
class DomainWidgetState extends State<DomainList> with AutomaticKeepAliveClientMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final visibleDomains = list.toList(growable: false);
    return widget.shrinkWrap
        ? SingleChildScrollView(child: Column(children: visibleDomains))
  ...
  void addBatch(Iterable<({Channel channel, HttpRequest request})> entries) {
    var hasNewDomain = false;
    final changedDomains = <DomainRequests>{};
    for (final entry in entries) {
      final request = entry.request;
      final host = request.remoteDomain();
      if (host == null) continue;

      final existing = containerMap[host];
      final domainRequests = getDomainRequests(request);
      if (existing == null) hasNewDomain = true;
      domainRequests.addRequest(request.requestId, request, sortDesc, notify: false);
      changedDomains.add(domainRequests);

      if (searchModel?.isNotEmpty == true && searchModel?.filter(request, null) == true) {
        searchView[host]?.addRequest(request.requestId, request, sortDesc, notify: false);
      }
    }
    for (final domain in changedDomains) {
      domain.changeState();
    }
    if (hasNewDomain && mounted) setState(() {});
  }
  ...
  void addResponses(Iterable<({ChannelContext context, HttpResponse response})> entries) {
    for (final entry in entries) {
      addResponse(entry.context, entry.response);
    }
  }
...
class DomainRequests extends StatefulWidget {
  ...
  ///添加请求
  void addRequest(String? requestId, HttpRequest request, bool sortDesc, {bool notify = true}) {
