...
class RequestRewriteState extends State<RequestRewriteWidget> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
            title: Text('Mock 规则·${localizations.requestRewrite}',
  ...
  Future<void> createGroup() async {
    final name = await _showGroupNameDialog(context, title: '新建 Mock 合集');
    if (name == null || name.isEmpty) return;
    if (widget.requestRewrites.groups.containsKey(name)) {
      if (mounted) FlutterToastr.show('合集已存在', context);
      return;
    }

    widget.requestRewrites.addGroup(name);
    await MultiWindow.invokeRefreshRewrite(Operation.groupAdd, group: name);
    if (mounted) setState(() {});
  }
}

Future<String?> _showGroupNameDialog(BuildContext context, {required String title, String? initialValue}) async {
  ...
  final result = await showDialog<String>(
    ...
    builder: (dialogContext) => AlertDialog(
      ...
      content: TextField(
        ...
        autofocus: true,
        decoration: const InputDecoration(labelText: '合集名称', hintText: '例如：示例订阅应用 · 权限测试'),
...
class _RequestRuleListState extends State<RequestRuleList> {
  ...
  @override
  Widget build(BuildContext context) {
    final grouped = <String, List<MapEntry<int, RequestRewriteRule>>>{};
    for (final group in widget.requestRewrites.groups.keys) {
      grouped[group] = [];
    }
    for (final entry in widget.requestRewrites.rules.asMap().entries) {
      grouped.putIfAbsent(widget.requestRewrites.groupNameFor(entry.value), () => []).add(entry);
    }
                ...
                child: SingleChildScrollView(
                  ...
                  Column(
                      children: grouped.entries
                          .expand((entry) => [groupHeader(entry.key, entry.value.length), ...rows(entry.value)])
                          .toList())
                ])))));
  }

  Widget groupHeader(String group, int count) {
    final enabled = widget.requestRewrites.groups[group] ?? true;
    final canEdit = group != RequestRewriteManager.ungroupedName;
    return Container(
      height: 38,
      ...
      child: Row(
        children: [
          Icon(enabled ? Icons.folder_open_outlined : Icons.folder_off_outlined, size: 19),
          const SizedBox(width: 8),
          Expanded(
            child: Text('$group ($count)', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
          ),
          const Text('合集开关', style: TextStyle(fontSize: 12)),
          SwitchWidget(
            scale: 0.65,
            value: enabled,
            onChanged: (value) async {
              widget.requestRewrites.setGroupEnabled(group, value);
              await MultiWindow.invokeRefreshRewrite(Operation.groupEnabled, group: group, enabled: value);
              if (mounted) setState(() {});
            },
          ),
          if (canEdit)
            PopupMenuButton<String>(
              ...
              itemBuilder: (_) => const [
                ...
                PopupMenuItem(value: 'delete', child: Text('删除合集（保留规则）')),
              ],
            )
          else
  ...
  Future<void> renameGroup(String group) async {
    final newName = await _showGroupNameDialog(context, title: '重命名 Mock 合集', initialValue: group);
    if (newName == null || newName.isEmpty || newName == group) return;
    if (widget.requestRewrites.groups.containsKey(newName)) {
      if (mounted) FlutterToastr.show('合集已存在', context);
      return;
    }
    widget.requestRewrites.renameGroup(group, newName);
    await MultiWindow.invokeRefreshRewrite(Operation.groupRename, group: group, newGroup: newName);
    if (mounted) setState(() {});
  }

  Future<void> deleteGroup(String group) async {
    await showConfirmDialog(context, content: '删除合集“$group”？组内规则会移到“${RequestRewriteManager.ungroupedName}”。',
        onConfirm: () async {
      widget.requestRewrites.removeGroup(group);
      await MultiWindow.invokeRefreshRewrite(Operation.groupDelete, group: group);
      if (mounted) setState(() {});
    });
  }
  ...
  void showGlobalMenu(Offset offset) {
    showContextMenu(context, offset, items: [
      ...
      PopupMenuItem(height: 35, child: const Text('移动到合集'), onTap: () => moveToGroup(selected.keys.toList())),
  ...
  Future<void> moveToGroup(List<int> indexes) async {
    if (indexes.isEmpty) return;
    final group = await showDialog<String>(
      context: context,
      builder: (dialogContext) => SimpleDialog(
        title: Text('移动 ${indexes.length} 条规则到合集', style: const TextStyle(fontSize: 16)),
        children: widget.requestRewrites.groups.keys
    ...
    for (final index in indexes) {
      ...
      rule.group = group == RequestRewriteManager.ungroupedName ? null : group;
      await MultiWindow.invokeRefreshRewrite(Operation.update, index: index, rule: rule);
    }
    if (mounted) {
  ...
  List<Widget> rows(List<MapEntry<int, RequestRewriteRule>> entries) {
    ...
    return entries.map((entry) {
      ...
      return InkWell(
          ...
          child: Container(
              ...
              child: Row(
                children: [
                  SizedBox(width: 130, child: Text(rule.name ?? '', style: const TextStyle(fontSize: 13))),
                  SizedBox(
                      width: 40,
                      child: SwitchWidget(
                          scale: 0.6,
                          ...
                          onChanged: (val) {
                            rule.enabled = val;
                            MultiWindow.invokeRefreshRewrite(Operation.update, index: index, rule: rule);
                          })),
                  const SizedBox(width: 20),
                  Expanded(
                      child: Text(rule.url, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 13))),
                  SizedBox(
                      width: 100,
                      child: Text(isCN ? rule.type.label : rule.type.name.camelCaseToSpaced(),
  ...
  void showMenus(TapDownDetails details, int index) {
    ...
    showContextMenu(context, details.globalPosition, items: [
      ...
      PopupMenuItem(height: 35, onTap: () => moveToGroup([index]), child: const Text('移动到合集')),
      PopupMenuItem(
          height: 35,
...
class _RewriteRuleEditState extends State<RewriteRuleEdit> {
  ...
  late TextEditingController groupInput;
  ...
  void initState() {
    ...
    groupInput = TextEditingController(text: rule.group);
        ...
        actions: [
          ...
          FilledButton(
              ...
              onPressed: () async {
                ...
                rule.group = groupInput.text.trim().isEmpty ? null : groupInput.text.trim();
