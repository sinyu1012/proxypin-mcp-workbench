...
class Har {
  ...
  static Future<List<HttpRequest>> readFile(File file) async {
    ...
    for (var index = 0; index < lines.length; index++) {
      ...
      try {
        final har = jsonDecode(value);
        list.add(toRequest(har));
      } on FormatException catch (error, stackTrace) {
        final isLastNonEmptyLine = lines.skip(index + 1).every((line) => line.trim().isEmpty);
        if (!isLastNonEmptyLine) rethrow;
        logger.w(
          '忽略未完整写入的抓包归档尾行: ${file.path}',
          error: error,
